...
export class SqliteStateDatabaseMissingError extends Schema.TaggedError<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start T2 once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedError<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.t2 database. Use an isolated --base-dir.";
  }
}
...
const t3SqliteStateCommand = Command.make(
  "t2-sqlite-state",
  {
    ...
    baseDir: Flag.String("base-dir").pipe(
      Flag.withDescription("Explicit T2 base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated T2 SQLite database with automatic backups for writes.",
  ),
);
