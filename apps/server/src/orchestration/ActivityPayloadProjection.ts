...
function projectPreviewToolMetadata(data: Record<string, unknown>, status: unknown) {
  ...
  if (
    typeof name !== "string" ||
    !/^(?:mcp__)?(?:t3-code|t3_code|t2code)_{1,2}preview_(?:open|navigate|status|snapshot|click|type|press|scroll|resize|set_appearance|evaluate|wait_for|recording_start|recording_stop)$/.test(
      name,
    )
  )
    return {};
