...
>()("@t2code/cli/diagnostics/ProcessDiagnostics") {}
    ...
    function* (input) {
      if (input.pid === process.pid) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some("Refusing to signal the T2 server process."),
        };
      }
      ...
      if (!canSignalCategory(selected.category)) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some(`Process ${input.pid} is not a signalable T2 backend descendant.`),
        };
      }
