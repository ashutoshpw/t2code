...
import { ServerSelfUpdateError, ThreadId } from "@t2code/contracts";
import { HostProcessExecutablePath } from "@t2code/shared/hostProcess";
    ...
    run: (input) =>
      Effect.gen(function* () {
        if (input.command === "npm") {
          ...
          const entry = path.join(prefix, "node_modules", "@t2code", "cli", "dist", "bin.mjs");
...
it.layer(NodeServices.layer)("server self update", (it) => {
  ...
  it.effect("rejects invalid versions and desktop-managed servers before staging", () =>
    Effect.gen(function* () {
      ...
      ).toBe("'latest' is not an exact t2code version.");
