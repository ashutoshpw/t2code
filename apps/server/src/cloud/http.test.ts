  ...
  it.effect("requires stored CLI authorization without exposing an HTTP endpoint", () =>
    Effect.gen(function* () {
      ...
      expect(error).toMatchObject({
        ...
        message: "Run `t2code connect link` to authorize this environment.",
      });
    }).pipe(
...
describe("releaseManagedTunnelOnShutdown", () => {
  ...
  it.effect("does not recreate a link that was unlinked while startup registration retried", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(yield* reconcileDesiredCloudLinkIfStillDesired("http://127.0.0.1:3772")).toBeNull();
  ...
  it.effect("still releases a pending update when the launcher is stopping", () => {
    // `t2code service uninstall` or `systemctl stop` during the pending window:
  ...
  it.effect("registers an existing tunnel and starts the confirmed connector", () => {
    ...
    return Effect.gen(function* () {
      expect(yield* registerManagedCloudTunnelRecovery("http://127.0.0.1:3772")).toMatchObject({
        status: "ready",
      });
      expect(requests).toHaveLength(1);
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        expect(yield* startManagedCloudTunnelIfOriginConfirmed("http://127.0.0.1:3772")).toBe(true);
        expect(applyConfigCalls).toEqual([config]);
        expect(requests).toEqual([]);
      }).pipe(provideReleaseHarness({ store, applyConfigCalls, requests }));
    },
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        expect(
          yield* startManagedCloudTunnelIfOriginConfirmed("http://127.0.0.1:3772", {
            requireConfirmedOrigin: false,
          }),
        ).toBe(true);
        expect(applyConfigCalls).toEqual([config]);
  ...
  it.effect.each(["replaced", "removed"] as const)(
    ...
    (mutation) => {
      ...
      return Effect.gen(function* () {
        expect(yield* registerManagedCloudTunnelRecovery("http://127.0.0.1:3772")).toEqual({
          status: "superseded",
        });
        expect(applyConfigCalls).toEqual([]);
        expect(values.has(CLOUD_ENDPOINT_CONFIRMED_ORIGIN)).toBe(false);
  ...
  it.effect("requests startup recovery for a legacy config without a recorded tunnel ID", () => {
    ...
    return Effect.gen(function* () {
      const registration = yield* registerManagedCloudTunnelRecovery("http://127.0.0.1:3772");
  ...
  it.effect("recovers a web-linked tunnel with its environment credential", () => {
    ...
    return Effect.gen(function* () {
      expect(yield* recoverManagedCloudTunnel("http://127.0.0.1:3772")).toBe(true);
      expect(requests).toHaveLength(1);
      expect(requests[0]?.method).toBe("POST");
  ...
  it.effect("allows managed tunnel provisioning to take longer than ten seconds", () =>
    Effect.gen(function* () {
      ...
      const recovery = yield* recoverManagedCloudTunnel("http://127.0.0.1:3772").pipe(
        provideReleaseHarness({
          store,
          applyConfigCalls,
          requests,
  ...
  it.effect("does not recover an environment without a managed tunnel credential", () => {
    ...
    return Effect.gen(function* () {
      expect(yield* recoverManagedCloudTunnel("http://127.0.0.1:3772")).toBe(false);
      expect(applyConfigCalls).toEqual([]);
      expect(requests).toEqual([]);
    }).pipe(provideReleaseHarness({ store, applyConfigCalls, requests }));
  });
  ...
  it.effect("ignores recovery requests for a tunnel that has already been replaced", () => {
    ...
    return Effect.gen(function* () {
      expect(
        yield* recoverManagedCloudTunnel("http://127.0.0.1:3772", {
          providerKind: "cloudflare_tunnel",
  ...
  it.effect.each([
    ...
    return Effect.gen(function* () {
      const error = yield* Effect.flip(recoverManagedCloudTunnel("http://127.0.0.1:3772"));
...
describe("link proof provider kinds", () => {
  ...
  it("accepts managed and manual endpoints but not t2_relay", () => {
    ...
    expect(isSupportedLinkProviderKind(proofRequest("t2_relay"))).toBe(false);
  });
