...
/** File in the logs dir that receives the service's stdout and stderr. `t2code triage` points agents at it. */
...
export class BootServiceInstallError extends Schema.TaggedError<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the T2 Code background service.";
  }
}
...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    case "user-manager-unavailable":
      return "Cannot reach the systemd user manager. Run `systemctl --user status` in a login session for the service user. Install your distribution's systemd user-session support if it is missing; do not run T2 with sudo.";
    ...
    case "linger-disabled":
      return 'Lingering is disabled. T2 Code will stop when your last login session ends and will not start at boot. Run `sudo loginctl enable-linger "$(id -un)"` on this machine, then retry the service command as your normal user.';
...
export interface BootServiceStatus {
  ...
  /**
   * The T2 home the installed unit serves. The unit name is fixed per user,
