...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@t2code/contracts";
import { fromLenientJson } from "@t2code/shared/schemaJson";
...
>()("@t2code/cli/vcs/VcsProjectConfig") {}
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".t2code", "vcs.json");
