...
import { ClaudeSettings, ProviderInstanceId } from "@t2code/contracts";
...
const ClaudeTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "t2code-claude-text-generation-test-",
...
function withFakeClaudeEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    ...
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "t2code-claude-text-" });
