import { OpenCodeSettings, ProviderInstanceId, TextGenerationError } from "@t2code/contracts";
...
const OpenCodeTextGenerationTestLayer = Layer.succeed(
  ...
  Layer.provideMerge(
    ServerConfig.ServerConfig.layerTest(process.cwd(), {
      prefix: "t2code-opencode-text-generation-test-",
    }),
  ),
...
const OpenCodeTextGenerationExistingServerTestLayer = Layer.succeed(
  ...
  Layer.provideMerge(
    ServerConfig.ServerConfig.layerTest(process.cwd(), {
      prefix: "t2code-opencode-text-generation-existing-server-test-",
    }),
  ),
  Layer.provideMerge(NetService.layer),
  Layer.provideMerge(NodeServices.layer),
