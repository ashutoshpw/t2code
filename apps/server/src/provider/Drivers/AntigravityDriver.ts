...
import { AntigravitySettings, ProviderDriverKind, ProviderSetupError } from "@t2code/contracts";
import { HostProcessPlatform } from "@t2code/shared/hostProcess";
import {
  NodeRuntimeUnavailableError,
  nodeRuntimeUnavailableMessage,
} from "@t2code/shared/nodeRuntime";
    ...
    Effect.gen(function* () {
      ...
      const makeDisposableRuntime = Effect.fn("AntigravityDriver.makeDisposableRuntime")(function* (
        ...
        const runtime = yield* makeRuntime({
          cwd,
          clientInfo: { name: "t2-code-provider-setup", version: "0.0.0" },
          mcpServers: [],
      ...
      const textGeneration = yield* makeAntigravityTextGeneration({
        ...
        makeRuntime: (cwd) =>
          makeRuntime({
            cwd,
            clientInfo: { name: "t2-code-text", version: "0.0.0" },
            mcpServers: [],
          }),
      });
