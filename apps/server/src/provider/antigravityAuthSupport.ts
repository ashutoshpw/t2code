...
/** Recognizes native auth failures and interactive login blocked by T2. */
...
function antigravityEnvironment(
  ...
  // Point it at a T2-owned directory so the driver can reclaim the space.
...
/**
 * The agent reads its user-global skills under `GEMINI_HOME`, which T2 points
...
export const prepareAntigravityProfile = Effect.fn("prepareAntigravityProfile")(function* (input: {
  ...
  if (
    ...
    return yield* authSupportError(
      "The T2 runtime path cannot be used to suppress Antigravity browser launches.",
    );
  }
...
/** Receives native 1.1.1 sign-in URLs and T2 browser-helper URLs without logging stderr. */
