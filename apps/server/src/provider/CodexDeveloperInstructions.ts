...
const T2_CODE_BROWSER_TOOL_INSTRUCTIONS = `

## T2 Code collaborative browser

You are running inside T2 Code. The \`t3-code\` MCP server is the product-native collaborative browser shared with the user. When it exposes \`preview_*\` tools, prefer those tools for browser navigation, inspection, interaction, screenshots, and recordings.
...
const T2_CODE_DEVICE_TOOL_INSTRUCTIONS = `
 ...
 */
const browserToolInstructions = (availability: boolean | T2CodeToolAvailability): string => {
  const tools = normalizeAvailability(availability);
  return `${tools.browser ? T2_CODE_BROWSER_TOOL_INSTRUCTIONS : ""}${
    tools.device ? T2_CODE_DEVICE_TOOL_INSTRUCTIONS : ""
  }`;
};

const codexPlanModeDeveloperInstructions = (
  browserToolsAvailable: boolean | T2CodeToolAvailability,
...
export function buildCodexDeveloperInstructions(
   ...
   */
  browserToolsAvailable: boolean | T2CodeToolAvailability = true,
): string {
  const base =
