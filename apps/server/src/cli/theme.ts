...
/**
 * `t2code theme` - inspect and set the environment's theme. Connected web and
 ...
 * Settings afterwards sticks until the next `t2code theme set`.
 *
...
import { BUILT_IN_THEME_IDS, UNPUBLISHABLE_THEME_IDS } from "@t2code/shared/themePalettes";
...
export class ThemeFileInvalidError extends Schema.TaggedError<ThemeFileInvalidError>()(
  ...
  override get message(): string {
    return `${this.filePath} is not a valid theme file. Use a theme exported from T2 Code, or a seeded file with name, appearance, canvas, and accent.`;
  }
}
...
export class ThemeTargetMissingError extends Schema.TaggedError<ThemeTargetMissingError>()(
  ...
  override get message(): string {
    return "Provide a theme id or file, or run `t2code theme clear` to remove the theme.";
  }
}

const envT2Home = Config.String("T2CODE_HOME").pipe(Config.option);
...
const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  ...
  const envHome = Option.filter(yield* envT2Home, (value) => value.trim().length > 0);
...
const themeSetCommand = Command.make("set", {
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // make `t2code theme set ocean` publish ./ocean whenever the cwd happens to
