...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `t2code connect link` to authorize and enable T3 Connect."
    : !status.desired
      ? "Run `t2code connect link` to enable T3 Connect."
      : !status.linked
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (background) {
          ...
          yield* Console.log(
            platform === "darwin"
              ? "\n✓ Background service ready\n\nT2 Code is set to run while you are logged in to this Mac. The server establishes the T3 Connect link on startup."
              : "\n✓ Background service ready\n\nT2 Code is set to keep running after you log out. The server establishes the T3 Connect link on startup.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
