...
const belongsToBootService = Effect.fn("cli.update.belongs_to_boot_service")(function* (
  ...
  if (platform === "linux") {
    ...
    return Option.isSome(cgroup) && cgroup.value.includes("/t2code.service");
  }
  if (platform === "darwin") {
...
const runUpdate = Effect.fn("cli.update.run")(function* (input: {
  ...
  // The unit name is per user, not per T2 home. Only touch the service when it
  ...
  if (serviceInstalled && !serviceCurrent) {
    yield* Console.log(
      "  A background service is installed for this T2 home. Restarting it interrupts anything running in it: agent turns, terminals, remote clients.",
    );
    if (input.assumeYes) {
      restartService = true;
  ...
  if (serviceUpdated) {
    ...
    yield* Console.log(
      `  The background service serves ${status.installedBaseDir ?? "another T2 home"} and was left unchanged.`,
    );
  }
