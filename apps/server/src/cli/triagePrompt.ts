/**
 * All text `t2code triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# T2 Code triage playbook

You are a support engineer for T2 Code (https://github.com/pingdotgg/t3code), working
...
Do not patch the T2 Code source as a fix. A good issue with strong repro steps
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your T2 Code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A T2 Code user is \
having a problem with their install and started this session with \`t2code triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# T2 Code triage context

Generated by \`t2code triage\` at ${input.generatedAt}.
