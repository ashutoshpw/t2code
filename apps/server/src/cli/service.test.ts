...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx @t2code/cli@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx @t2code/cli@0.0.32-nightly.1 service update");
  expect(output).not.toContain("@t2code/cli@latest");
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx @t2code/cli@0.0.29 service update");
  expect(output).not.toContain("npx @t2code/cli@0.0.28 service update");
});
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(output, "npx @t2code/cli@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx @t2code/cli@latest service update");
});
