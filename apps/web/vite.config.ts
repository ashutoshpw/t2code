...
import { DEV_PROXIED_PATH_PREFIXES } from "@t2code/shared/devProxy";
...
const isSingleOriginDev = process.env.T2CODE_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_T2CODE_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.T2CODE_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: T2CODE_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); T2CODE_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.T2CODE_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // T2CODE_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.T2CODE_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.T2CODE_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_T2CODE_RELAY_URL": JSON.stringify(configuredRelayUrl),
