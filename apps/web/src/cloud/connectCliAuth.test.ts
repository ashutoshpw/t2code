...
describe("connectCliAuth", () => {
  ...
  it("requires both the publishable key and the CLI OAuth client id", () => {
    ...
    vi.stubEnv("VITE_CLERK_JWT_TEMPLATE", "t2-relay");
    vi.stubEnv("VITE_T2CODE_RELAY_URL", "https://relay.example.com");
  ...
  it("sends the sign-in redirect to the authorize endpoint, not back to /connect", () => {
    ...
    const connectUrl =
      "https://app.t2.codes/connect#state=state-1&challenge=challenge-1&port=34338";
