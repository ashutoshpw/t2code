...
import { ManagedRelay, relayProtectedErrorMessage } from "@t2code/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `T2 Code cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `T2 Code cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
// independently of T2 Connect while clients reach the environment out of band.
