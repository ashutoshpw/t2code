import { EnvironmentId, ProjectId, type AgentSessionProjectCandidate } from "@t2code/contracts";
...
describe("groupOnboardingProjects", () => {
  it("groups clones by origin, keeps local repos separate, and folds non-git folders away", () => {
    const main = candidate("/code/t2code", {
      git: github("pingdotgg/t2code"),
      threadCount: 79,
      lastActiveAt: "2026-08-21T12:00:00.000Z",
    });
    const clone = candidate("/code/clones/t2code-2", {
      git: github("pingdotgg/t2code"),
      threadCount: 13,
    ...
    expect(
      ...
      {
        label: "pingdotgg/t2code",
        paths: ["/code/t2code", "/code/clones/t2code-2"],
        threadCount: 92,
        lastActiveAt: "2026-08-21T12:00:00.000Z",
      },
      {
