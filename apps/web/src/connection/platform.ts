...
import { bootstrapRemoteBearerSession } from "@t2code/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@t2code/client-runtime/environment";
import { managedRelayAccountChanges, managedRelaySessionAtom } from "@t2code/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@t2code/client-runtime/rpc";
import {
    ...
    const cloudSession = CloudSession.of({
      ...
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to T2 Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The T2 Connect session is unavailable.",
          });
        }
        return token;
      }),
