import type { ServerProvider, ServerProviderVersionAdvisory } from "@t2code/contracts";
...
export function getProviderSummary(provider: ServerProvider | undefined) {
  ...
  if (!provider.enabled || provider.status === "disabled") {
    return {
      headline: "Disabled",
      detail:
        provider.message ?? "This provider is installed but disabled for new sessions in T2 Code.",
    };
  }
