...
/**
 * Each member's decoded t2.json, so file-backed settings show the file as a
...
function useMemberProjectFiles(scope: ReturnType<typeof resolveSettingsScope>) {
  ...
  return useAtomValue(
    useMemo(
      () =>
        Atom.make((get) => {
          const files = new Map<string, T2ProjectFile | null>();
          for (const member of members) {
            ...
            files.set(
              ...
              data === null || data.truncated ? null : parseT2ProjectFile(data.contents),
            );
          }
          return files;
        }),
