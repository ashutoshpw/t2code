...
describe("fileBreadcrumbs", () => {
  it("builds project, directory, and file crumbs", () => {
    expect(fileBreadcrumbs("t2code", "apps/web/src/main.tsx")).toEqual([
      { label: "t2code", path: "", kind: "project" },
  ...
  it("starts host paths outside the workspace at the filesystem root", () => {
    ...
    expect(fileBreadcrumbs("t2code", "\\\\server\\share\\report.md").map((c) => c.path)).toEqual([
      "\\\\server",
      "\\\\server\\share",
