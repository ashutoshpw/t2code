import type { EnvironmentId, ServerSelfUpdateCapability } from "@t2code/contracts";
    ...
    if (isDesktopAppUpdate) {
      ...
      const confirmed =
        (await requestConfirmDialog(
          `Update the T2 Code desktop app that runs the ${serverLabel}? It will close and relaunch on that machine.`,
        )) ?? true;
      if (!confirmed) {
        return;
      }
    ...
    try {
      ...
      toastManager.add({
        ...
        description: isDesktopAppUpdate
          ...
          : `Reconnected on @t2code/cli@${result.value.targetVersion}.`,
      });
    } finally {
