...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "T2 Code");
  expect(findCaptureDestination([window(99, "T2 Code"), target], 42, "T2 Code")).toBe(target);
  expect(findCaptureDestination([window(99, "T2 Code")], 42, "T2 Code")).toBeUndefined();
