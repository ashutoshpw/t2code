  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["@t2code/cli#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-browser-secret.mjs && node scripts/build-preview-annotation-css.mjs && cross-env T2CODE_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["@t2code/cli#build"],
        cache: false,
      },
      "dev:bundle": {
