import type { DesktopSshPasswordPromptRequest } from "@t2code/contracts";
import { DesktopSshPasswordPromptResolutionInputSchema } from "@t2code/contracts";
import type { SshPasswordRequest } from "@t2code/ssh/auth";
...
export class DesktopSshPromptWindowUnavailableError extends Schema.TaggedErrorClass<DesktopSshPromptWindowUnavailableError>()(
  ...
  override get message(): string {
    ...
    return `T2 Code window is unavailable during ${this.stage} for SSH authentication to ${this.destination} (request: ${request}).`;
  }
}
...
>()("@t2code/desktop/ssh/DesktopSshPasswordPrompts") {}
