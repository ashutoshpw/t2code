...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "t2-kde-test-"));
  paths = {
...
it("captures with verified identity, cleans private files, and activates only the owning T2 PID/title", async () => {
  await setup.perform("install-kde-helper");
