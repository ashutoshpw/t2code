...
export class CaptureShortcutConfig {
  ...
  async apply(id: string, desktop: Target["desktop"]): Promise<DesktopCaptureConfigApplied> {
    ...
    const temporary = NodePath.join(directory, `.t2-capture-${NodeCrypto.randomUUID()}.tmp`);
    let staged = false;
    try {
      ...
      await unchanged();
      const backupPath = `${root.resolvedPath}.t2-capture-backup-${NodeCrypto.randomUUID()}`;
