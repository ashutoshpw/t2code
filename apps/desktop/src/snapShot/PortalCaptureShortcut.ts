...
import type { SnapShotKeyChord } from "@t2code/contracts";
...
export class PortalCaptureShortcut {
  ...
  async configure() {
    ...
    if (!this.hasSession || this.version < 2)
      throw new Error(
        "Open your desktop's shortcut settings and allow T2 Code's capture shortcut.",
      );
    await this.call({
  ...
  private failed = (error: unknown) => {
    ...
    this.update({
      ...
      shortcutMessage: this.managedByHyprland
        ? "Couldn't connect to Hyprland shortcuts. Make sure xdg-desktop-portal-hyprland is running, then restart T2 Code."
          ...
          this.update({
            ...
            shortcutMessage:
              this.version >= 2
                ...
                : "Shortcut permission wasn't granted. Allow T2 Code in your desktop's shortcut settings.",
          });
          return undefined;
        }
    ...
    if (this.managedByHyprland) {
      this.update({
        ...
        shortcutMessage: shortcut
          ...
          : "Hyprland did not register the capture action. Check that xdg-desktop-portal-hyprland is running, then restart T2 Code.",
      });
      return;
    }
