...
it("does not activate T2 until requested, then matches PID and title", async () => {
  ...
  windows = [
    { ...window, id: 1, pid: 999, title: "T2 Code" },
    { ...window, id: 2, pid: process.pid, title: "Other T2" },
    { ...window, id: 3, pid: process.pid, title: "T2 Code" },
  ];
  await snapshot.feedback!.activate("T2 Code");
...
it("waits for the restored T2 window to map instead of polling", async () => {
  ...
  handler = async (request, socket) => {
    ...
    if (request === "EventStream")
      send(socket, {
        WindowOpenedOrChanged: { window: { ...window, id: 4, pid: process.pid, title: "T2 Code" } },
      });
  };
  await snapshot.feedback!.activate("T2 Code");
...
it("rejects ambiguous activation targets", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  windows = [1, 2].map((id) => ({ ...window, id, pid: process.pid, title: "T2 Code" }));
  await expect(snapshot.feedback!.activate("T2 Code")).rejects.toThrow("More than one");
});
...
it("cancels pending activation when capture feedback is closed", async () => {
  ...
  const activation = expect(snapshot.feedback!.activate("T2 Code")).rejects.toThrow("cancelled");
