...
describe("findAccessibleWindow", () => {
  ...
  it.each(["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"])(
    ...
    (frame) => {
      const windows = [{ name: `${frame} t2code`, bounds: captured.bounds }];

      expect(findAccessibleWindow(windows, { ...captured, title: "⠋ t2code" }, "wayland")).toBe(
        windows[0],
      );
    },
  );
  ...
  it("rejects matching spinners when the window sizes differ", () => {
    expect(
      findAccessibleWindow(
        [{ name: "⠙ t2code", bounds: { ...captured.bounds, width: 400 } }],
        { ...captured, title: "⠋ t2code" },
        "wayland",
      ),
    ).toBeUndefined();
  });
  ...
  it("rejects ambiguous normalized titles even if one matches the captured spinner exactly", () => {
    const windows = [
      { name: "⠋ t2code", bounds: captured.bounds },
      { name: "⠙ t2code", bounds: captured.bounds },
    ];

    expect(
      findAccessibleWindow(windows, { ...captured, title: "⠋ t2code" }, "wayland"),
    ).toBeUndefined();
  });

  it("keeps exact title matching outside Wayland", () => {
    expect(
      findAccessibleWindow([{ name: "⠙ t2code", bounds: captured.bounds }], {
        ...captured,
        title: "⠋ t2code",
      }),
    ).toBeUndefined();
