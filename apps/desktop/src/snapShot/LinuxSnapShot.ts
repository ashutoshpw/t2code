...
const EXTENSION = "org.gnome.Shell.Extensions.T2SnapShot";
const EXTENSION_PATH = "/org/gnome/Shell/Extensions/T2SnapShot";
...
export class LinuxCaptureConnection {
  ...
  async captureExtension(appId: string, options?: FeedbackOptions): Promise<LinuxWindowSnapshot> {
    ...
    if (result !== RequestNameReply.PRIMARY_OWNER) {
      throw new Error("Another T2 Code instance is capturing a window. Try again.");
    }
