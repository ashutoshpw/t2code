...
import { buildRemoteNodeEnvScript } from "@t2code/ssh/tunnel";
...
>()("@t2code/desktop/wsl/DesktopWslEnvironment") {}
...
// that install wrote.
const WSL_RUNTIME_READY_MARKER = ".t2code-wsl-runtime-ready";
const WSL_RUNTIME_SELECTED_MARKER = ".t2code-wsl-runtime-selected";
...
const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_BINARY_MISSING_EXIT_CODE
    ? "WSL support is missing from this T2 Code build: the packaged Linux node-pty binary was not included. Install a build that includes WSL support."
    : null;
