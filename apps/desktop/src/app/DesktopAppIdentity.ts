...
const AppPackageMetadata = Schema.Struct({
  t2codeCommitHash: Schema.optional(Schema.String),
});
...
>()("@t2code/desktop/app/DesktopAppIdentity") {}
...
export const resolveUserDataPath = Effect.map(
  DesktopEnvironment.DesktopEnvironment,
  (environment) => environment.path.join(environment.appDataDirectory, environment.userDataDirName),
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.t2codeCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
