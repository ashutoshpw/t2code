...
import { HostProcessUserId } from "@t2code/shared/hostProcess";
  ...
  const server = NodeNet.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "T2 Code could not process the desktop app request."),
        );
      });
    });
...
>()("@t2code/desktop/app/DesktopAppActivation") {}
