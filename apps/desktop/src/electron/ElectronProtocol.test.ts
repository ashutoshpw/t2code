...
describe("ElectronProtocol", () => {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          assert.include(
            ...
            "script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval' https://clerk.t2.codes https://challenges.cloudflare.com",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
          ...
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "img-src 'self' t2code-dev: blob: data: http: https:",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
  ...
  it("keeps executable sources host-restricted while allowing runtime network resources", () => {
    ...
    assert.deepEqual(directives["media-src"], ["'self'", "t2code:", "blob:", "http:", "https:"]);
    assert.deepEqual(directives["frame-src"], ["'self'", "blob:", "http:", "https:"]);
    assert.deepEqual(directives["font-src"], ["'self'", "t2code:", "data:"]);
  });
});
