...
describe("tryOpenExternalUrl", () => {
  it("opens supported URLs", async () => {
    ...
    await expect(
      tryOpenExternalUrl("https://github.com/pingdotgg/t2code", "pull-request"),
    ).resolves.toBe(true);
  });
  ...
  it("logs stable URL context without exposing the opening failure", async () => {
    ...
    await expect(
      tryOpenExternalUrl("https://github.com/pingdotgg/t2code/pull/1?token=secret", "pull-request"),
    ).resolves.toBe(false);
