...
import { PROVIDER_SEND_TURN_MAX_ATTACHMENTS } from "@t2code/contracts";
...
describe("native pasted image cleanup", () => {
  ...
  it("recognizes only files created in the native composer paste directory", () => {
    expect(
      isOwnedPastedImageUri(
        "file:///private/var/mobile/Containers/Data/Application/app/tmp/t2-composer-paste/id.png",
      ),
    ).toBe(true);
    expect(isOwnedPastedImageUri("file:///private/var/mobile/photos/id.png")).toBe(false);
    expect(isOwnedPastedImageUri("https://example.com/t2-composer-paste/id.png")).toBe(false);
  });
  ...
  it("deletes rejected and overflow owned files without deleting user-owned files", async () => {
    const rejected =
      "file:///private/var/mobile/Containers/Data/Application/app/tmp/t2-composer-paste/bad.png";
    const overflow =
      "file:///private/var/mobile/Containers/Data/Application/app/tmp/t2-composer-paste/overflow.png";
  ...
  it("persists folded text unchanged in the app-owned attachment directory", async () => {
    ...
    expect(attachment).toEqual({
      ...
      fileUri: "file:///documents/t2-composer-attachments/attachment-id-pasted-text.txt",
