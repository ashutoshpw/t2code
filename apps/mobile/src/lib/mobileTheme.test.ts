...
describe("mobile themes", () => {
  it("declares every runtime theme variable in the static stylesheet", () => {
    const generatedVariables = createMobileThemeVariables(T2_CHAT_THEME.colors, "light");
  ...
  it.each(["light", "dark"] as const)(
    ...
    (appearance) => {
      const variables = getMobileThemeVariables("t2-code", appearance);
      expect(
      ...
      for (const platform of ["ios", "android"]) {
        const runtime = getMobileThemeRuntimeVariables("t2-code", appearance, platform);
  ...
  it.each(["light", "dark"] as const)(
    ...
    (appearance) => {
      const variables = getMobileThemeVariables("t2-code", appearance);
      const desktop =
        appearance === "dark" ? T2_CODE_DARK_THEME_COLORS : T2_CODE_LIGHT_THEME_COLORS;
  ...
  it("changes either theme without switching the active appearance", () => {
    const themeIds = { light: "t2-chat", dark: "grove" } as const;
  ...
  it("maps semantic palette roles onto every mobile color variable", () => {
    const variables = createMobileThemeVariables(T2_CHAT_THEME.colors, "light");
    expect(variables["--color-sheet-solid"]).toBe(
      themeColorToNativeColor(T2_CHAT_THEME.colors.chrome),
    );
    expect(variables["--color-warning"]).toBe(
      themeColorToNativeColor(T2_CHAT_THEME.colors.warningSurface),
    );
    expect(variables["--color-warning-foreground"]).toBe(
      themeColorToNativeColor(T2_CHAT_THEME.colors.warningForeground),
    );
