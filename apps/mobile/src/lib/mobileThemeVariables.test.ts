...
describe("mobile theme runtime variables", () => {
  it("matches the standard base palette to the generated stylesheet", () => {
    expect(getMobileThemeRuntimeVariables("t2-code", "light", "web")).toEqual(
      readDefaultMobileThemeVariables("light"),
    );
    expect(getMobileThemeRuntimeVariables("t2-code", "dark", "web")).toEqual(
    ...
    (themeId) => {
      for (const appearance of ["light", "dark"] as const) {
        const base = getMobileThemeVariables(
          themeId === "material-you" ? "t2-code" : themeId,
          appearance,
        );
        ...
        expect(android).toEqual({
          ...
          "--color-header": themeColorWithAlpha(
            base[
              themeId === "t2-code" || themeId === "material-you"
                ? "--color-row-hover"
                : "--color-drawer"
            ],
            1,
  ...
  it.each(["t2-code", "material-you"] as const)(
    ...
    (themeId) => {
      for (const appearance of ["light", "dark"] as const) {
        ...
        if (appearance === "light") {
          ...
          expect(ios).toEqual(getMobileThemeVariables("t2-code", appearance));
