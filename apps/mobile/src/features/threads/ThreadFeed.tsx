...
import { resolveViewedImageAsset } from "@t2code/client-runtime/work-log/presentation";
import {
  renderCodexFileCitationsAsMarkdown,
  splitCodexArtifactTemplateMarkdown,
} from "@t2code/client-runtime/codex-markdown-directives";
import { CHAT_LIST_ANCHOR_OFFSET, resolveChatListAnchoredEndSpace } from "@t2code/shared/chatList";
import { imageMimeType } from "@t2code/shared/image";
...
import { markdownFileIconSource } from "@t2code/mobile-markdown-text/file-icons";
import { PierreEntryIcon } from "../../components/PierreEntryIcon";
import { markdownLinkIconSource } from "@t2code/mobile-markdown-text/link-icons";
import {
  normalizeNativeMarkdownUrl,
  resolveMarkdownInlineCodePresentation,
  resolveMarkdownLinkIcon,
  resolveMarkdownLinkPresentation,
} from "@t2code/mobile-markdown-text/links";
import {
      ...
      >
        ...
        >
          ...
          <View className="min-w-0 flex-1 gap-1">
            <Text className="font-t2-medium text-sm text-foreground" numberOfLines={2}>
              {attachment.name}
            </Text>
...
function MarkdownInlineCode(props: {
  ...
  return (
    <NativeText
      className={presentation ? "font-t2-bold" : "font-mono"}
  ...
  return (
    <View className="my-2 min-w-0 flex-row items-center gap-3 rounded-2xl border border-border bg-card px-3 py-3">
      ...
      <View className="min-w-0 flex-1">
        <Text className="font-t2-bold text-sm text-foreground" numberOfLines={1}>
      ...
      {props.onUse ? (
        ...
        >
          <Text className="font-t2-bold text-xs text-foreground">Use template</Text>
        </Pressable>
      ) : null}
    </View>
  );
    ...
    return (
      ...
      >
        <Text
          ...
          className="font-t2-medium text-sm tabular-nums text-foreground-muted"
        >
          {entry.label}
        </Text>
        <ThreadDisclosureChevron
      ...
      >
        ...
        <View className="shrink-0 flex-row items-center gap-1.5">
          <SymbolView
            ...
            type="monochrome"
          />
          <Text className="font-t2-medium text-xs text-foreground-muted">{label}</Text>
        </View>
        <View className="h-px flex-1 bg-subtle" />
      </View>
    );
      ...
      return (
        <View className="mb-5 items-end">
          ...
          <View className="mt-1 flex-row items-center justify-end gap-1 pr-0.5">
            <Text className="font-t2-medium text-xs tabular-nums text-foreground-secondary">
  ...
  return (
    ...
    >
      <View className="max-w-[320px] items-center gap-2">
        <Text className="text-center font-t2-bold text-lg text-foreground">{props.title}</Text>
