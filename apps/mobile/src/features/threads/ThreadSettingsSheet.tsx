...
function ProviderHeader(props: {
  ...
  const content = (
    <>
      ...
      <Text className="text-sm font-t2-medium text-foreground-muted">{props.label}</Text>
      {props.collapsible ? (
        <>
          <View className="flex-1" />
          {props.collapsed ? (
            <Text className="text-2xs font-t2-medium text-foreground-muted">
              {props.modelCount}
            </Text>
          ) : null}
          <SymbolView
...
function DisclosureRow(props: {
  ...
  return (
    ...
    >
      <Text className="text-sm font-t2-medium text-foreground">{props.label}</Text>
      <View className="flex-1" />
      {props.value ? (
...
function ThreadSettingsOptionsItem(props: {
  ...
  return (
    <View style={{ paddingBottom: insets.bottom + bottomToolbarInset + 12 }}>
      <Text className="px-5 pb-2 pt-2 text-sm font-t2-medium text-foreground-muted">Options</Text>
      <Animated.View
      ...
      {Platform.OS !== "ios" && session.hasLegacyModels ? (
        <>
          <Text className="px-5 pb-2 pt-7 text-sm font-t2-medium text-foreground-muted">
            Catalog
          </Text>
