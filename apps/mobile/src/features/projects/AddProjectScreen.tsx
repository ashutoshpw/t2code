...
function SectionTitle(props: { readonly children: string }) {
  return (
    <Text
      className={
        Platform.OS === "android"
          ? "px-4 text-sm font-t2-medium text-primary-text"
          : "px-1 text-2xs font-t2-bold tracking-[0.7px] uppercase text-foreground-muted"
      }
    >
      {props.children}
    </Text>
    ...
    >
      <View className="flex-row items-center gap-3">
        ...
        <View className="flex-1 gap-0.5">
          <Text className="text-base leading-snug font-t2-bold">{props.title}</Text>
          {props.subtitle ? (
  ...
  return (
    ...
    >
      {props.loading ? (
        ...
        <Text className="text-base font-t2-bold text-primary-foreground">{props.label}</Text>
      )}
    </Pressable>
  );
}
...
function EmptyEnvironmentState() {
  ...
  return (
    <View className="items-center gap-3 rounded-2xl bg-card px-5 py-8">
      <Text className="text-center text-lg font-t2-bold">Environment unavailable</Text>
      ...
      >
        <Text className="text-sm font-t2-bold text-primary-foreground">Add environment</Text>
      </Pressable>
    </View>
  );
}
    ...
    <AddProjectShell title="Clone destination">
      ...
      {repositoryTitle ? (
        <View className="rounded-[24px] bg-card px-4 py-3">
          <Text className="text-base font-t2-bold">{repositoryTitle}</Text>
