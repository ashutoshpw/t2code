...
import { BUILT_IN_THEMES, getThemeColorsForAppearance } from "@t2code/shared/themePalettes";
...
describe("getMobileTerminalTheme", () => {
  it("uses the shared default light terminal colors", () => {
    expect(getMobileTerminalTheme("t2-code", "light")).toMatchObject({
      background: "#fcfcfc",
      foreground: "#27272a",
  ...
  it("uses the shared default dark terminal colors", () => {
    expect(getMobileTerminalTheme("t2-code", "dark")).toMatchObject({
      background: "#0a0a0a",
      foreground: "#f5f5f5",
  ...
  it("applies the selected palette without replacing ANSI status colors", () => {
    const standard = getMobileTerminalTheme("t2-code", "dark");
...
describe("buildGhosttyThemeConfig", () => {
  it("serializes theme colors into a ghostty config file", () => {
    const config = buildGhosttyThemeConfig(getMobileTerminalTheme("t2-code", "dark"));
