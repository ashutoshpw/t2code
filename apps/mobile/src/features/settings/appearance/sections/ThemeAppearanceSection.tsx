...
import { mixThemePreviewBase, THEME_PREVIEW_RENDER_SPECS } from "@t2code/shared/themePreview";
...
function ThemeCard(props: {
  ...
  return (
    <View className="min-w-36 flex-1 basis-[47%] gap-3 rounded-[24px] border border-border bg-grouped-card px-2 py-4">
      ...
      <Text
        className="min-w-0 flex-1 px-1 text-lg font-t2-medium"
        numberOfLines={1}
        pointerEvents="none"
      >
        {props.label}
  ...
  return (
    ...
    >
      ...
      <Text
        className={
          props.selected
            ? "text-center text-base font-t2-bold text-foreground"
...
function SectionLabel({ children }: { readonly children: string }) {
  return <Text className="px-2 text-sm font-t2-medium text-foreground-muted">{children}</Text>;
}
