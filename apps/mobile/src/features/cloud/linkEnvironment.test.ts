...
import { RelayMobileClientId } from "@t2code/contracts/relay";
import { DPOP_UNKNOWN_HINT, ManagedRelay } from "@t2code/client-runtime/relay";
import { remoteHttpClientLayer } from "@t2code/client-runtime/rpc";
...
describe("mobile cloud link environment client", () => {
  ...
  it.effect("reuses the status-and-connect token when connecting from the cloud list", () =>
    Effect.gen(function* () {
      ...
      expect(environmentTokenBody.get("client_label")).toBe("T2 Code Mobile");
