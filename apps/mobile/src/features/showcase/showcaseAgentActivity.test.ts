...
const projects = [
  project("moonbase-terminal", "t3code", "T2 Code"),
...
it("stages relay-shaped rows against the seeded threads", () => {
  ...
  assert.deepStrictEqual(
    ...
    [
      ["pocket-command-center", "Approval", "T2 Code", "2026-07-16T08:59:00.000Z"],
      ["beautiful-boot", "Input", "Linux", "2026-07-16T08:56:00.000Z"],
      ["buttery-suspense", "Working", "React", "2026-07-16T08:58:00.000Z"],
      ["remote-command-center", "Done", "T2 Code", "2026-07-16T08:57:00.000Z"],
    ],
  );
  assert.strictEqual(
...
it("encodes the Android Live Update and its alert like a relay push", () => {
  ...
  assert.strictEqual(
    ...
    "Approval\tPut the command center in your pocket\tT2 Code",
  );
  assert.strictEqual(data.activity_line_3, "Done\tMake remote coding feel local\tT2 Code");
  assert.strictEqual(data.alert_title, "Put the command center in your pocket");
  assert.strictEqual(data.alert_body, "Approval: T2 Code");
