...
export function EmptyState(props: {
  ...
  if (props.variant === "plain") {
    return (
      <View className="items-center px-8 py-8">
        <Text className="text-center text-xl font-t2-bold text-foreground">{props.title}</Text>
  ...
  return (
    <View className="rounded-[22px] border border-border bg-card p-5">
      <Text className="font-t2-bold text-lg text-foreground">{props.title}</Text>
      ...
      {props.action ? (
        ...
        <Pressable
          ...
          onPress={props.onAction}
        >
          <Text className="text-sm font-t2-bold text-primary-foreground">{props.actionLabel}</Text>
        </Pressable>
      ) : null}
    </View>
  );
