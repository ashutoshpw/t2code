...
> [!WARNING]
> T2 Code Mobile is currently in development and is not distributed yet. If you want to try it out, you can build it from source.
...
- `development`: Expo dev client, installable side-by-side as `T2 Code Dev`
- `preview`: persistent internal preview build, installable side-by-side as `T2 Code Preview`
- `production`: store/release build as `T2 Code`

Run commands from `apps/mobile`.

T2 Connect is optional and disabled in a fresh clone. Public configuration belongs in the
...
For preview or production EAS environments, set `T2CODE_CLERK_PUBLISHABLE_KEY`,
`T2CODE_CLERK_JWT_TEMPLATE`, and `T2CODE_RELAY_URL`
