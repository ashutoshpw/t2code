# T2 Mobile Terminal Native Module
...
custom-I/O and live-padding branch. `T2TerminalView` owns a `libghostty` surface and uses that
callback I/O model:
...
`t2code/custom-io-ordered-feed` branch when vendored, based on VVTerm's
`vvterm/custom-io-padding` branch).

```bash
apps/mobile/modules/t2-terminal/scripts/build-libghostty-ios16.sh
```
...
```bash
apps/mobile/modules/t2-terminal/scripts/build-libghostty-android.sh
```
