...
#import "T2MarkdownTextShadowNode.h"
#import "T2MarkdownTextComponentDescriptor.h"
#import "T2MarkdownTextRun.h"
#import "T2ContextChip.h"
...
@interface T2ContextChipAccessibilityElement : UIAccessibilityElement
@property(nonatomic, weak) T2MarkdownTextRun *run;
@end

@implementation T2ContextChipAccessibilityElement
...
@interface T2ContextCopyTextView : UITextView
@property(nonatomic, copy) NSDictionary *contextClipboardConfig;
@end

@implementation T2ContextCopyTextView
...
static void T2MarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<T2MarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void T2MarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<T2MarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = T2MarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        T2MarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
    NSDictionary *chip = T2ContextChipPayload(imageUri);
    if (chip != nil) {
      CGSize size = CGSizeMake(attachmentRange.chipWidth, attachmentRange.chipHeight);
      attachment.bounds = T2ContextChipBounds(runAttributes[NSFontAttributeName], size);
      NSString *iconUri = [chip[@"iconUri"] isKindOfClass:NSString.class] ? chip[@"iconUri"] : nil;
      attachment.image = T2ContextChipImage(chip, size, iconUri ? images[iconUri] : nil);
    }
    ...
    [attributedString replaceCharactersInRange:range
                          withAttributedString:T2MarkdownTextAttachmentString(attachment, runAttributes)];
  }
}

@protocol T2MarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface T2MarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<T2MarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<T2MarkdownOutsideTapTarget>)target;

@end

static const void *T2MarkdownOutsideTapCoordinatorKey =
    &T2MarkdownOutsideTapCoordinatorKey;

@implementation T2MarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<T2MarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<T2MarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, T2MarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<T2MarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static T2MarkdownOutsideTapCoordinator *
T2MarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  T2MarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, T2MarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[T2MarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        T2MarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface T2MarkdownText () <RCTT2MarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface T2MarkdownText () <T2MarkdownOutsideTapTarget>
@end

@implementation T2MarkdownText {
  UIView * _view;
  T2ContextCopyTextView * _textView;
  T2MarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<T2MarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const T2MarkdownTextProps>();
    _props = defaultProps;
    ...
    _textView = [[T2ContextCopyTextView alloc] init];
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    T2MarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, T2MarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [T2MarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<T2MarkdownTextProps const>(_props);
  ...
  T2MarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  T2MarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:[T2MarkdownTextRun class]]) {
      continue;
    }

    T2MarkdownTextRun *textChild = (T2MarkdownTextRun *)child;
  ...
  if (textChanged) {
    ...
    for (auto it = _state->getData().attachmentRanges.rbegin();
      ...
      NSDictionary *chip = T2ContextChipPayload([NSString stringWithUTF8String:it->imageUri.c_str()]);
  ...
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:T2MarkdownTextRun.class]) continue;
    T2MarkdownTextRun *run = (T2MarkdownTextRun *)child;
    run.contextChipInteractive = NO;
  }
  for (const auto &attachmentRange : _state->getData().attachmentRanges) {
    NSDictionary *chip = T2ContextChipPayload(
    ...
    T2MarkdownTextRun *run = [self childForCharacterRange:range];
    ...
    T2ContextChipAccessibilityElement *element =
        [[T2ContextChipAccessibilityElement alloc] initWithAccessibilityContainer:self];
    element.run = run;
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::T2MarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::T2MarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<T2MarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
...
{
  const auto &oldViewProps = *std::static_pointer_cast<T2MarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<T2MarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == T2MarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == T2MarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == T2MarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == T2MarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const T2MarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (nullable T2MarkdownTextRun *)childForCharacterRange:(NSRange)characterRange
{
...
- (T2MarkdownTextRun*)getTouchChild:(CGPoint)location
{
...
{
  T2MarkdownTextRun *child = [self childForCharacterRange:textItem.range];
  if (![child hasContextMenu] && !child.contextChipInteractive) {
    return defaultAction;
  }

  __weak T2MarkdownTextRun *weakChild = child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::T2MarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::T2MarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> T2MarkdownTextCls(void)
{
  return T2MarkdownText.class;
}
