...
import expo.modules.t2markdowntext.T2ContextChip
import org.json.JSONObject
import kotlin.math.max

class T2ComposerEditorView(context: Context, appContext: AppContext) : ExpoView(
  context,
  appContext
) {
        ...
        ComposerChipSpan(
          T2ContextChip(
            ...
            colors = T2ContextChip.Colors(
              accent = T2ContextChip.color(token.accent, chipTheme.chipText),
...
private class ComposerChipSpan(
  private val chip: T2ContextChip
) : ReplacementSpan() {
...
internal class SelectionAwareEditText(context: Context) : EditText(context) {
  ...
  private fun copyContext(cut: Boolean): Boolean {
    ...
    T2ComposerClipboard.write(context, text.substring(start, end), clipboardFragment)
