    ...
    Effect.gen(function* () {
      ...
      const configured = Effect.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({ T2CODE_RELAY_CLIENT_CONFIG_ENV: target }),
        ),
      );
      ...
      expect(first).toContain("T2CODE_RELAY_URL=https://relay.example.com\n");
      expect(first).toContain("T2CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-v1\n");
      expect(first).toContain("T2CODE_MOBILE_OTLP_TRACES_TOKEN=mobile-v1\n");
      ...
      expect(third).toContain("T2CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-v2\n");
