...
export const relayClientConfigEnv = (config: RelayClientConfig & { readonly url: string }) =>
  ({
    ...
    T2CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: Redacted.value(config.clientTracingToken),
  }) as const;
 ...
 * deploy. Set `T2CODE_RELAY_CLIENT_CONFIG_ENV` to write elsewhere (CI does).
 */
...
export const PublishClientConfig = Alchemy.Action(
  ...
  Effect.gen(function* () {
    ...
    const override = yield* Config.String("T2CODE_RELAY_CLIENT_CONFIG_ENV").pipe(Config.option);
