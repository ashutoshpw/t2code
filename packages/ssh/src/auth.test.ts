...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      assert.equal(env.T2_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "t2code");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\t2code-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\t2code-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
