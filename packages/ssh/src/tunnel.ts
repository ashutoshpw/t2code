...
if command -v t2code >/dev/null 2>&1; then
  exec t2code "$@"
fi
# npm extracts a package before it runs the native builds of its dependencies,
# so a failed build (t2code depends on node-pty, which needs a C toolchain) leaves
# the npx cache without a t2code executable. \`npx --yes\` then exits 0 without
...
require_installed_t3_cli() {
  if ! T3_CLI_PATH="$("$@" -- sh -c 'command -v t2code')"; then
  ...
  printf 'Remote host installed %s but npm produced no t2code executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@T3_PACKAGE_SPEC@@ >&2
  return 1
}
...
printf 'Remote host is missing the t2code CLI and could not install @@T3_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "@t2code/cli@latest");
