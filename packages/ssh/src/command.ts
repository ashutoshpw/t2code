...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@t2code/contracts";
...
export function resolveRemoteT3CliPackageSpec(input: {
  ...
  if (!input.isDevelopment && PUBLISHABLE_T3_VERSION_PATTERN.test(appVersion)) {
    return `@t2code/cli@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "@t2code/cli@nightly";
  }

  return input.updateChannel === "nightly" ? "@t2code/cli@nightly" : "@t2code/cli@latest";
}
