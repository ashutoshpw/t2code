...
describe("ssh tunnel scripts", () => {
  it("builds the remote t2code runner with npx and npm fallbacks", () => {
    const script = buildRemoteT3RunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "T3_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec t2code "$@"');
    assert.include(script, 'exec "$T3_CLI_PATH" "$@"');
    assert.include(script, "could not install '@t2code/cli@latest'");
    assert.include(script, "require_installed_t3_cli npx --yes --package '@t2code/cli@latest'");
    assert.include(
      script,
      "require_installed_t3_cli npm exec --yes --package '@t2code/cli@latest'",
    );
    assert.include(script, "npm produced no t2code executable");
  ...
  it("shell-quotes package specs in the remote t2code runner", () => {
    const script = buildRemoteT3RunnerScript({
      packageSpec: "@t2code/cli@nightly; touch /tmp/t3-owned",
    });

    assert.include(
      script,
      "require_installed_t3_cli npx --yes --package '@t2code/cli@nightly; touch /tmp/t3-owned'",
    );
    assert.notInclude(script, "exec npx --yes @t2code/cli@nightly; touch /tmp/t3-owned");
  });

  it("builds the remote t2code runner with a node script override", () => {
    const script = buildRemoteT3RunnerScript({
  ...
  it("uses the remote t2code runner for launch and pairing scripts", () => {
    ...
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "@t2code/cli@nightly" }),
      "@t2code/cli@nightly",
    );
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "@t2code/cli@nightly" }),
      "@t2code/cli@nightly",
    );
    assert.include(
