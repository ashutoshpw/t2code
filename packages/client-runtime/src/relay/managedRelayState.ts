...
import { RelayEnvironmentConnectScope, RelayEnvironmentStatusScope } from "@t2code/contracts/relay";
import { decodeRelayJwt } from "@t2code/shared/relayJwt";
      ...
      return yield* Effect.tryPromise({
        try: () => readCachedClerkToken(nowMillis),
          ...
          new ManagedRelaySessionError({
            message: "Could not obtain the T2 Connect session token.",
            cause,
          }),
      });
    }),
  ...
  return session.readClerkToken().pipe(
    Effect.flatMap((token) =>
      token
        ...
        : Effect.fail(
            new ManagedRelaySessionError({
              message: "The T2 Connect session token is unavailable.",
            }),
          ),
    ),
  );
...
export const deregisterManagedRelayEnvironment = Effect.fn(
  ...
  if (!session || session.accountId !== input.accountId) {
    return yield* new ManagedRelaySessionError({
      message: "Sign in to T2 Connect before deregistering an environment.",
    });
  }
...
function requireClerkToken(
  ...
  if (!session || session.accountId !== accountId) {
    return Effect.fail(
      new ManagedRelaySessionError({
        message: "Sign in to T2 Connect before loading relay data.",
      }),
    );
  }
    ...
    error =
      isManagedRelayRequestFailedError(cause) && cause.relayError
        ...
        : cause instanceof Error
          ? cause.message
          : "Could not load T2 Connect data.";
