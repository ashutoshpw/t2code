...
> = {
  ...
  forgejo: {
    ...
    unauthenticated:
      "Authenticate your Forgejo or Gitea server with `fj --host <server-url> auth add-token` on the T2 Code server. If fj is missing or unconfigured for that server, use `tea login add`. A configured fj account must be repaired with fj.",
  },
  gitlab: {
    missing:
  ...
  bitbucket: {
    missing:
      "Bitbucket needs API credentials on the server. Set T2CODE_BITBUCKET_EMAIL and T2CODE_BITBUCKET_API_TOKEN, or T2CODE_BITBUCKET_ACCESS_TOKEN.",
    unauthenticated:
      "Bitbucket rejected the configured credentials. Check T2CODE_BITBUCKET_EMAIL and T2CODE_BITBUCKET_API_TOKEN.",
  },
};
