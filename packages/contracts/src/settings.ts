...
/**
 * The environment's theme, set with `t2code theme set <id>`. Each client applies
...
// Deliberately absent from ServerSettingsPatch: `t2code theme set` checks that an
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let T2 Code spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
   ...
   * withholds the MCP credential, so the `t2-code` server (and with it every
   ...
   * of the same value from one they already applied: `t2code theme set` must act
