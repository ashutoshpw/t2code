...
const CLOUDFLARED_PATH_ENV_NAME = "T2CODE_CLOUDFLARED_PATH";
  ...
  const installUnlocked = Effect.fn("cloudflared.installUnlocked")(function* (
    ...
    if (!releaseAsset) {
      return yield* new RelayClientInstallError({
        ...
        message: `T2 Code does not provide a managed relay client binary for ${platform}-${arch}.`,
      });
    }
