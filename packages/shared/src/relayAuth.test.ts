...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.t2.codes"))).toBe(
      "clerk.t2.codes",
    );
    expect(clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.t2.codes"))).toBe(
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.t2.codes/path";
