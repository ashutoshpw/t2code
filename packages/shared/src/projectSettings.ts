 ...
 * t2.json for keys in `PROJECT_FILE_BACKED_SETTINGS`, then the built-in
 * default (reported as "environment", since that is what the environment
 * value is when nothing set it).
 */
export type ProjectSettingSource = "environment" | "project" | "t2.json";
...
/**
 * With the checkout's decoded t2.json (or null for a missing or invalid
...
export function resolveProjectSettings(
  ...
  projectFile: T2ProjectFile | null,
 ...
 * but only see the file later, such as the git driver reading the t2.json of
