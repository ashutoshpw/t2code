...
describe("threadPullRequestKeysEqual", () => {
  ...
  it("ignores host and repository case", () => {
    expect(
      threadPullRequestKeysEqual(
        { host: "GitHub.com", repository: "PingDotGG/t2code", number: 1 },
        { host: "github.com", repository: "pingdotgg/t2code", number: 1 },
      ),
    ).toBe(true);
    expect(
      threadPullRequestKeysEqual(
        { host: "github.com", repository: "pingdotgg/t2code", number: 1 },
        { host: "gitlab.com", repository: "pingdotgg/t2code", number: 1 },
      ),
    ).toBe(false);
  });
});
...
describe("resolveThreadCurrentPullRequest", () => {
  ...
  it("reports a stack when several links are open and puts the highest layer on top", () => {
    const stack = {
      ...
      id: "s1",
      number: 1,
      url: "https://github.com/pingdotgg/t2code/stacks/1",
      base: "main",
      layers: [
  ...
  it.each([
    {
      ...
      url: "https://github.enterprise.test/pingdotgg/t2code/pull/7",
    },
...
describe("threadPullRequestSearchTerms", () => {
  it("includes completed and unsynced links but excludes dismissed links", () => {
    ...
    expect(terms).toContain("pingdotgg/t2code#12");
    expect(terms).toContain("https://github.com/pingdotgg/t2code/pull/12");
...
it("searches the legacy projection when old environments decode to an empty links list", () => {
  const linkedPullRequest = {
    ...
    repository: "pingdotgg/t2code",
    number: 12,
    url: "https://github.com/pingdotgg/t2code/pull/12",
  };
