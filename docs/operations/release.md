...
- `T2CODE_WEB_ROUTER_URL`: defaults to `https://app.t2.codes`.
- `T2CODE_WEB_LATEST_DOMAIN`: defaults to `latest.app.t2.codes`.
- `T2CODE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.t2.codes`.
...
- Repository slug source:
  - `T2CODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
...
2. Create an explicit App ID for `codes.t2.desktop` and enable Associated Domains.
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.codes.t2.desktop` and includes
    Associated Domains.
