...
- The web build emits `third-party-licenses.json` beside `index.html`. The Settings page loads that
  static file, so the same artifact works in hosted web, the client bundled with `npx @t2code/cli`, and
  desktop.
...
workspace packages, and omits first-party `@t2code/*` packages. The web manifest starts from the
