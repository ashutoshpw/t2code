...
Self-contained builds install as a download from the T2 Code GitHub release
...
run `t2code service install`. It follows the stable train by default; set
...
and are never offered as updates; the installer and `t2code update` only take you
there when you ask for the channel explicitly, and warn you when they do.

Once a self-contained `t3` is installed, `t2code update` moves the machine to a
...
`t2code service update`. From a script there is no prompt, so pass `--yes` to
...
yourself. Pass an exact version (`t2code update 0.0.41-preview.20260912.1595`) to
...
`t2code uninstall` reverses the install script: it shows what it found (the
...
T2 Connect can offer service installation during setup, but the two are managed
separately. Signing out of T2 Connect does not stop or uninstall the service.
