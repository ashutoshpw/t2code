...
T2 Code uses Claude Code's login and configuration. Start with the default provider
...
Select the model you want in T2 Code. For an OpenRouter model outside the built-in
...
do not replace the explicit model ID selected in T2 Code. Custom models may have
