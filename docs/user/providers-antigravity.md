...
T2 Code runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for T2 Code to confirm
...
hostname. Only that T2 Code sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. T2 Code's separate Plan mode
...
T2 Code keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. T2 Code also reads `.gemini/skills` and the
