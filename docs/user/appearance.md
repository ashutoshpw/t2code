...
On web and desktop, choose **Create theme** to adjust a palette, or import a T2 Code or VS Code
...
takes precedence. If the server stops publishing the selected theme, T2 Code falls back to its
standard theme.

Run this on the server to set a default and switch connected clients to it:

```bash
t2code theme set nightfall
```

Clients that are offline apply it when they reconnect. Each client applies the setting once;
choosing another theme afterward sticks until the next `t2code theme set`. Run the command again to
reapply it, even if the name is unchanged.

`t2code theme clear` removes the default without changing anyone's current theme. `t2code theme show` lists
...
Set `appearance` to `light` or `dark` and supply hex colors for `canvas` and `accent`. T2 Code
