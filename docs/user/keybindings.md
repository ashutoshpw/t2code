...
`~/.t2/userdata/keybindings.json` by default. You can edit this file directly.
...
T2 Code creates the file with its defaults and adds new defaults on later startups.
...
Invalid rules are ignored; if the file cannot be parsed, T2 Code uses defaults.
