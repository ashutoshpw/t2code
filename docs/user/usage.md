...
databases, including T2-managed profiles. Set `OPENCODE_DATA_DIR` or `ANTIGRAVITY_DATA_DIR` on the
...
This includes headless T2 sessions and desktop usage across machines; the same account counts
once across connected environments. Without an accessible CLI login, T2 shows a
notice instead of incomplete local totals. T2 does not estimate missing tokens from conversation text.
On macOS, choose **Enable Cursor usage** on Usage to allow T2 to read your existing CLI login
...
the environment. T2 cannot report limits for external OpenCode servers because their credentials
...
Weekly, or both for each provider. Reopen T2 to refresh expired readings. The Android widget
