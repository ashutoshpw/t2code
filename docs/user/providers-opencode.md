...
T2 Code requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let T2 Code start OpenCode locally. A password in
provider settings applies to both that server and T2 Code's connection. With no
...
settings. T2 Code uses only that configured password for an external server; it
...
restart before T2 Code can see configuration changes.
