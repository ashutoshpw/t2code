...
`agent-device` command line, which T2 Code sets up for them.
...
Studio's Device Manager. T2 Code detects standard SDK locations; set
...
Agents drive the device through the `agent-device` command line. T2
...
local network, Tailscale, and T2 Connect. Live video needs a secure page
...
T2 checks common Homebrew and Android SDK locations; custom installations need
...
sessions and stops its T2 helpers when reachable; simulators keep running.

T2 provides discovery, streaming, and control. Arrange app builds,
...
The connected T2 server manages the device hub and agent tools on its own machine and configured SSH hosts. Required versions install automatically the next time those tools are used. Settings → Integrations → Check device tool versions reads installed versions without installing tools or starting devices.

To receive newer tool versions on a remote environment, update that environment's T2 server. Updating only the browser or mobile app does not update the remote server. An offline host keeps its installed files, but an update needs network access before device support can start; T2 does not fall back to an older version. Reconnect the host and use Retry if installation fails. Existing device and agent-access settings are preserved.
