...
function runStep(stdout, exitCode = 0) {
  const runnerTemp = mkdtempSync(join(tmpdir(), "t2-relay-state-test-"));
  try {
...
for (const prefix of [
  ...
  test(`extracts tracing config ${prefix ? "after progress output" : "from plain JSON"}`, () => {
    ...
    assert.equal(
      result.envFile,
      "T2CODE_RELAY_CLIENT_OTLP_TRACES_URL=https://example.invalid/traces\n" +
        "T2CODE_RELAY_CLIENT_OTLP_TRACES_DATASET=fixture-dataset\n" +
        "T2CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=fixture-token\n",
    );
  });
}
