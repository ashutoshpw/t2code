...
**Web** is kind of two surfaces, as we have the public facing "app.t3.codes" as well as locally hosting the web app through the `npx @t2code/cli` command. Both need to be supported by all new features where reasonable.
...
2. **Writing to the live install.** `~/.t3/userdata` is the developer's real T2 Code database, in use while you work. Reading it and copying from it are fine, and a good way to get real test data (see Test data). Never start a server against it, never open it read-write, never clean it up.
...
- `vp run dev` starts server and web. In a worktree, state defaults to that worktree's gitignored `.t3`, which deliberately outranks an ambient `T2CODE_HOME` so you cannot land on shared state by accident. An explicit `--home-dir` still wins.
