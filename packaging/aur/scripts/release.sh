...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="T2-Code-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit t2code-icon.png
git config user.name 't2code-ci'
git config user.email 't2code-ci@users.noreply.github.com'
git add -A
